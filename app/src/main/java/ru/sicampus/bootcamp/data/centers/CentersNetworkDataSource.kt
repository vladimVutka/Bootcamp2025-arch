...
class CentersNetworkDataSource {
    suspend fun getCenters(
        ...
        runCatching {
            val result = Network.client.get("http://192.168.0.121:8080/api/1.0/organization/organizations"){
            headers {
    ...
    suspend fun findByNameAdress(token: String, name: String) : Result<CentersDto> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            val result = Network.client.get("http://192.168.0.121:8080/api/1.0/organization/$encodedName") {
                headers {
                    append(HttpHeaders.Authorization, token)
                }
            }
